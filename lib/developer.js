...
async function parseDeveloperApps (html, opts) {
  ...
  const processedApps = map(scriptData.extractor(appsMappings), path(initialMappings.apps, html));
  const apps = opts.fullDetail
    ? await processFullDetailApps(processedApps, opts)
    : processedApps;

  const token = path(initialMappings.token, html);
