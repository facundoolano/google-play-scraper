...
async function parseDeveloperApps (html, opts) {
  ...
  // Some developer pages render without the app list section (e.g. pages
  // that only expose lazy-loaded content). R.path then returns undefined and
  // R.map used to crash with "Cannot read properties of undefined (reading
  // 'fantasy-land/map')" (#730). Treat a missing section as an empty list.
  const appsSection = R.path(initialMappings.apps, html);
  if (!Array.isArray(appsSection)) {
    return [];
  }

  const processedApps = R.map(scriptData.extractor(appsMappings), appsSection);
