...
function checkFinished (opts, savedApps, nextToken) {
  // loads fullDetail for all apps that are being returned (starting with last item)
  function load (num) {
    return app({ appId: savedApps[num - 1].appId }).then(x => {
      // save the result in savedApps
      savedApps[num - 1] = x;
      // if not finished for all wanted apps
      if (num > 1) {
        // load the next one
        return load(num - 1, savedApps);
      }
      // return the wanted slice
      return savedApps.slice(0, opts.num);
    });
  }
  if (savedApps.length >= opts.num || !nextToken) {
    if (opts.fullDetail) {
      return load(opts.num);
    }
    return savedApps.slice(0, opts.num);
  }
