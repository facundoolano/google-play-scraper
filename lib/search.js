...
const SPECIAL_COUNTRIES = ['kr', 'hk', 'tw'];
/*
 ...
 */
// FIXME this looks awfully similar to the other processAndRecur
function processAndRecur (html, opts, savedApps, mappings) {
  if (R.is(String, html)) {
    html = scriptData.parse(html);
  }

  const apps = appList.extract(mappings.apps, html, isSpecialCountry(opts.country));
  const token = isSpecialCountry(opts.country) ? null : R.path(mappings.token, html);
...
function checkFinished (opts, savedApps, nextToken) {
  ...
  const requestOptions = Object.assign({
    url: `${BASE_URL}/_/PlayStoreUi/data/batchexecute?rpcids=qnKhOb&bl=boq_playuiserver_20190424.04_p0&hl=${opts.lang}&gl=${opts.country}&authuser=0&soc-app=121&soc-platform=1&soc-device=1`,
    method: 'POST',
...
function initialRequest (opts) {
  ...
  return request(Object.assign({ url }, opts.requestOptions), opts.throttle)
    ...
    .then((html) => processAndRecur(html, opts, [], isSpecialCountry(opts.country) ? INITIAL_SPECIAL_MAPPINGS : INITIAL_MAPPINGS));
}
...
const INITIAL_SPECIAL_MAPPINGS = {
...
function isSpecialCountry (country) {
  return SPECIAL_COUNTRIES.indexOf(country.toLowerCase()) !== -1;
}
