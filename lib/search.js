...
async function processFirstPage (html, opts, savedApps, mappings) {
  ...
  const hasAboutTheseResults = R.is(String, R.path(mappings.aboutResultsTitle, html));
  if (hasAboutTheseResults) {
    R.path(mappings.aboutResultSection, html).shift();
  }
  ...
  if (sections.length === 1) {
    ...
    moreResultsSection = sections.filter(section => R.is(String, R.path(SECTIONS_MAPPING.title, section)))[0];
