...
function details (savedApps, num) {
  // console.log(savedApps[num].appId);
  return (app({ appId: savedApps[num].appId }));
}
...
function checkFinished (opts, savedApps, nextToken) {
  function load (num) {
    return details(savedApps, opts.num).then(x => {
      savedApps[num - 1] = x;
      // console.log(savedApps[opts.num]);
      // console.log(num);
      if (num > 1) {
