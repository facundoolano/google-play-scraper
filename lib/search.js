 ...
 */
function processAndRecur (html, opts, savedApps, rootPath) {
  if (R.is(String, html)) {
    html = scriptData.parse(html);
  }

  // console.log(JSON.stringify(html['ds:3']));
  const apps = mapping.processList([...rootPath, 0], html);
  const tokenPath = [...rootPath, 7, 1];
  const token = R.path(tokenPath, html);
...
function checkFinished (opts, savedApps, nextToken) {
  ...
  return request(requestOptions, opts.throttle)
      .then((html) => {
        ...
        return processAndRecur(data, opts, savedApps, [0, 0]);
      });
}
...
function initialRequest (opts) {
  ...
  return request(Object.assign({url}, opts.requestOptions), opts.throttle)
      ...
      .then((html) => processAndRecur(html, opts, [], ['ds:3', 0, 1, 0, 0]));
}
