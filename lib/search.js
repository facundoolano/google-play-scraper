...
function getFullDetail (apps, num) {
  return new Promise(function (resolve) {
    resolve(app({ appId: apps[num].appId }));
  });
}
...
function details (savedApps, num) {
  ...
  let x;
  for (i = 0; i < num; i++) {
    x = getFullDetail(savedApps.slice(0, num), i).then(function (result) {
      y.push(result);
    }).then(function () {
      return y;
    });
...
function checkFinished (opts, savedApps, nextToken) {
  if (savedApps.length >= opts.num || !nextToken) {
    if (opts.fullDetail) {
      let x = details(savedApps, opts.num);
      return x;
    } else {
