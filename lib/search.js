...
async function processFirstPage (html, opts, savedApps, mappings) {
  ...
  const sections = path(mappings.sections, html);
  if (noResultsFound(sections, opts)) return [];

  const moreResultsSection = sections.filter(section => isMoreSection(section))[0];
  const mainAppSection = path(mappings.app, html);

  const processedApps = map(scriptData.extractor(moreResultsMapping), path(SECTIONS_MAPPING.apps, moreResultsSection));
  if (mainAppSection) {
  ...
  const apps = opts.fullDetail
    ...
    : processedApps;
  const token = path(SECTIONS_MAPPING.token, moreResultsSection);

  return checkFinished(opts, [...savedApps, ...apps], token);
}

function isMoreSection (section) {
  const sectionTitle = path(SECTIONS_MAPPING.title, section);
  return is(String, sectionTitle);
}
...
function removeSectionsIfPathValueOfType (html, p, type) {
  if (is(type, path(p, html))) {
    path(INITIAL_MAPPINGS.sections, html).shift();
  }
}

function noResultsFound (sections, opts) {
  return sections.some(section => {
    const noResults = path(SECTIONS_MAPPING.noResults, section);
    return is(String, noResults) && noResults.endsWith(`<b>${opts.term}</b>`);
  });
}
