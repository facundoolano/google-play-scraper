...
function processAndRecur (html, opts, savedApps, mappings, initial = false) {
  if (R.is(String, html)) {
    html = scriptData.parse(html);
  }

  let apps = appList.extract(mappings.apps, html);
  // const token = R.path(mappings.token, html);

  if (apps.length === 0 && initial) {
    apps = appList.extract(SECOND_INITIAL_MAPPINGS.apps, html);
  }

  // return checkFinished(opts, [...savedApps, ...apps], token);
  return checkFinished(opts, [...savedApps, ...apps]);
}
...
function initialRequest (opts) {
  ...
  return request(Object.assign({ url }, opts.requestOptions), opts.throttle)
    ...
    .then((html) => processAndRecur(html, opts, [], INITIAL_MAPPINGS, true));
}
...
const SECOND_INITIAL_MAPPINGS = {
  apps: ['ds:4', 0, 1, 1, 22, 0]
};
