...
function parseSimilarApps (similarObject, opts) {
  ...
  // When the app page carries no similar-apps cluster at all (region
  // restricted apps, apps without recommendations), the extraction returns
  // null/undefined and `.length` used to crash with "Cannot read properties
  // of null (reading 'length')" (#701). Surface the intended domain error
  // instead so callers can distinguish "no similar apps" from a parser bug.
...
async function processFirstPage (html, opts, savedApps, mappings) {
  ...
  // Same guard as the cluster extraction above: a cluster page that renders
  // without the app list section must yield an empty result, not a ramda
  // crash on undefined input.
  const appsSection = R.path(mappings.apps, html);
  if (!Array.isArray(appsSection)) {
    return savedApps;
  }

  const processedApps = R.map(scriptData.extractor(mapping), appsSection);
...
// Exported for tests: exercises the missing-cluster guard without depending
// on which live apps currently lack a similar-apps section.
export { parseSimilarApps };
