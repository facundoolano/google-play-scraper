import { is, chain, map, path } from 'ramda';
import request from './utils/request.js';
import scriptData from './utils/scriptData.js';
import { BASE_URL, c } from './constants.js';
...
function processShortPermissionsData (html) {
  ...
  const permissionNames = chain(permission => permission[MAPPINGS.type], validPermissions);
...
function processPermissionData (html) {
  ...
  const permissions = Object.values(c.permission).reduce((permissionAccummulator, permission) => {
    ...
    permissionAccummulator.push(
      ...chain(flatMapPermissions, html[permission])
    );
...
function flatMapPermissions (permission) {
  const input = path(MAPPINGS.permissions, permission);
  ...
  return map(scriptData.extractor(mappings), input);
}
