import { path, includes, values, is, map } from 'ramda';
...
function validate (opts) {
  ...
  if (opts.sort && !includes(opts.sort, values(c.sort))) {
...
async function processReviewsAndGetNextPage (html, opts, savedReviews) {
  ...
  const parsedHtml = is(String, html)
  ...
  const token = path(REQUEST_MAPPINGS.token, parsedHtml);
