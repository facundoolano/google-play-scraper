...
// var html; request({url: 'https://play.google.com/store/apps/details?id=com.dxco.pandavszombies&hl=en&gl=us'}).then(cheerio.load).then((res) => {html=res}).catch(console.log)
...
// selectors with the new ugly class name should be declared as constants here
const DEVELOPER_SELECTOR = '.hrTbp';

function parseFields ($) {
  const title = $('[itemprop="name"]').text().trim();
  const developer = $(DEVELOPER_SELECTOR).html();
  const developerId = $(DEVELOPER_SELECTOR).attr('href').split('id=')[1];
  // the description meta tag now contains the full description instead of the summary
  // const summary = $('meta[name="description"]').attr('content');

  const genre = $('[itemprop="genre"]').first();
  const genreText = genre.text();
  const genreId = genre.attr('href').split('/')[4];
  ...
  const descriptionHTML = $('meta[name="description"]').attr('content');
...
function descriptionText (descriptionHtml) {
  // preserve the line breaks when converting to text
  const html = '<div>' + descriptionHtml.replace(/<\/p>/g, '\n</p>') + '</div>';
