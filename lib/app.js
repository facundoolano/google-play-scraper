...
function parseFields($) {
    ...
    var genreField = detailsInfo.find('span[itemprop="genre"]');
    var genre = [];
    var genreFieldLength = genreField.length;
    if ( genreFieldLength > 1 ) {
        genreField.each(function (i, elem) {
           genre[i] = $(this).text();
        });
        for (var i = 0; i < genre.length; i++) {
            if ( genre[i].indexOf("&") > -1 ) {
                var splitted = genre[i].split(" & ");
                genre.splice(i, 1);
                genre = genre.concat(splitted);
            }            
        }
        genre = genre.filter(function(element, pos) {
            return genre.indexOf(element) == pos;
        });
    } else {
        genre = detailsInfo.find('span[itemprop="genre"]').text();
    }
    ...
    var screens = $('.thumbnails').find('img').nextAll();
    var screenshots = [];
    for(var i = 0; i < screens.length; i++) {
        screenshots.push(screens[i].attribs.src);
    }
    return {
        title: title,
        icon: icon,
