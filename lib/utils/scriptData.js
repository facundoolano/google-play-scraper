...
function extractor (mappings) {
  return function extractFields (parsedData) {
    ...
    const isNewLayout = typeof R.path(mappings.title[0], parsedData) !== 'string';

    return R.map((spec) => {
      if (R.is(Array, spec)) {
        if (Array.isArray(spec[0])) {
          return R.path(spec[isNewLayout ? 1 : 0], parsedData);
        }
      ...
      if (spec.useServiceRequestId) {
        const input = extractDataWithServiceRequestId(parsedData, spec);
        return spec.fun(input, parsedData);
      }

      if (Array.isArray(spec.path[0])) {
        const value = R.path(spec.path[isNewLayout ? 1 : 0], parsedData);
        return spec.fun(value, parsedData);
      }
      return spec.fun(R.path(spec.path, parsedData), parsedData);
    }, mappings);
  };
}
