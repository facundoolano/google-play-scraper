...
function getListWithPermissions (opts) {
  return function (apps) {
    if (opts.withPermissions) {
      const promises = apps.map((app) => permissions({appId: app.appId, short: opts.permissionsShort, lang: opts.lang})
        .then((permissions) => R.assoc('permissions', permissions, app)));
      return Promise.all(promises);
    }
    return apps;
  };
...
module.exports = {getParseList, getListWithPermissions};
