...
function descriptionHtmlLocalized (searchArray) {
  const descriptionTranslation = path([12, 0, 0, 1], searchArray);
  const descriptionOriginal = path([72, 0, 1], searchArray);
...
function descriptionText (description) {
  ...
  const html = load('<div>' + description.replace(/<br>/g, '\r\n') + '</div>');
...
function extractComments (comments) {
  if (!comments) return [];
  return comments.map(path([4])).slice(0, 5);
}
...
function extractFeatures (featuresArray) {
  ...
  return features.map(feature => ({
    title: feature[0],
    description: path([1, 0, 0, 1], feature)
  }));
}
