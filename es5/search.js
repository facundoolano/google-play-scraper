...
var body = 'f.req=%5B%5B%5B%22qnKhOb%22%2C%22%5B%5Bnull%2C%5B%5B10%2C%5B10%2C50%5D%5D%2Ctrue%2Cnull%2C%5B96%2C27%2C4%2C8%2C57%2C30%2C110%2C79%2C11%2C16%2C49%2C1%2C3%2C9%2C12%2C104%2C55%2C56%2C51%2C10%2C34%2C77%5D%5D%2Cnull%2C%5C%22%token%%5C%22%5D%5D%22%2Cnull%2C%22generic%22%5D%5D%5D';
...
function processAndRecur(html, opts, savedApps, mappings) {
  ...
  var apps = appList.extract(mappings.apps, html);
  var token = R.path(mappings.token, html);

  return checkFinished(opts, [].concat(_toConsumableArray(savedApps), _toConsumableArray(apps)), token);
}
...
function initialRequest(opts) {
  ...
  function skipClusterPage(html) {
    var match = html.match(/href="\/store\/apps\/collection\/search_collection_more_results_cluster?(.*?)"/);
    if (match) {
      var innerUrl = 'https://play.google.com/' + match[0].split(/"/)[1];
  ...
  var url = 'https://play.google.com/store/search?c=apps&q=' + opts.term + '&hl=' + opts.lang + '&gl=' + opts.country + '&price=' + opts.price;
  return request(Object.assign({ url: url }, opts.requestOptions), opts.throttle).then(skipClusterPage).then(function (html) {
    return processAndRecur(html, opts, [], INITIAL_MAPPINGS);
  });
}
