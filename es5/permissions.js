...
var isPermission = function isPermission(item) {
  return R.is(Array, item) && item.length === 3 && R.is(String, item[0]) && R.is(String, item[1]);
};
...
function permissions(opts) {
  return new Promise(function (resolve, reject) {
    ...
    }).then(parsePermissions).then(function (perms) {
      if (opts.short) {
